...
generateArray = (island) => {
  ...
  let justBuyPrice = [];
      justBuyPrice.push(parseInt(this.state.Islands.OldZealand.buyPrice));
      var i = 0;
      for (i = 0; i < this.state.Islands.OldZealand.prices.length; i++){
        if (this.state.Islands.OldZealand.prices[i] !== 0) {
          justBuyPrice.push(parseInt(this.state.Islands.OldZealand.prices[i]));
        }
      }
...
// END UPDATE METHODS 
   ...
   render() {
       return (
         <>
         {this.state.Islands.OldZealand.prices.length > 0 &&
          <PredictionChart
