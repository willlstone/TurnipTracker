...
class LANDon extends React.Component {
...
//   if(this.state.Islands.LANDon.prices.length > 0){
...
// callAPI = async() => {
//   await this.generateRequest(this.state.Islands.LANDon);
// }
...
generateArray = (island) => {
  ...
  let justBuyPrice = [];
      justBuyPrice.push(parseInt(this.state.Islands.LANDon.buyPrice));
      var i = 0;
      for (i = 0; i < this.state.Islands.LANDon.prices.length; i++){
        if (this.state.Islands.LANDon.prices[i] !== 0) {
          justBuyPrice.push(parseInt(this.state.Islands.LANDon.prices[i]));
        }
      }
...
// END UPDATE METHODS 
   ...
   render() {
       return (
         <>
         {this.state.Islands.LANDon.prices.length > 0 &&
          <PredictionChart
