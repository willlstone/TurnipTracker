...
class Kokomo extends React.Component {
...
generateArray = (island) => {
  ...
  let justBuyPrice = [];
      justBuyPrice.push(parseInt(this.state.Islands.Kokomo.buyPrice));
      var i = 0;
      for (i = 0; i < this.state.Islands.Kokomo.prices.length; i++){
        if (this.state.Islands.Kokomo.prices[i] !== 0) {
          justBuyPrice.push(parseInt(this.state.Islands.Kokomo.prices[i]));
        }
      }
...
// END UPDATE METHODS 
   ...
   render() {
       return (
         <>
         {this.state.Islands.Kokomo.prices.length > 0 &&
          <PredictionChart
