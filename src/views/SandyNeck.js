...
class SandyNeck extends React.Component {
...
//   if(this.state.Islands.OldZealand.prices.length > 0){
...
// callAPI = async() => {
//   await this.generateRequest(this.state.Islands.OldZealand);
// }
...
generateArray = (island) => {
  ...
  let justBuyPrice = [];
      justBuyPrice.push(parseInt(this.state.Islands.SandyNeck.buyPrice));
      var i = 0;
      for (i = 0; i < this.state.Islands.SandyNeck.prices.length; i++){
        if (this.state.Islands.SandyNeck.prices[i] !== 0) {
          justBuyPrice.push(parseInt(this.state.Islands.SandyNeck.prices[i]));
        }
      }
...
// END UPDATE METHODS 
   ...
   render() {
       return (
         <>
         {this.state.Islands.SandyNeck.prices.length > 0 &&
          <PredictionChart
